...
class CustomArguments:
    ...
    test_dataset_name_custom: Optional[str] = field(
        default=None,
        metadata={
            "help": 'The name of the test split in the input dataset. If not specified, will use the "test" split when do_predict is enabled'
        },
    )
    train_split_name_custom: Optional[str] = field(
        default=None,
        metadata={
            "help": 'The name of the train split in the input dataset. If not specified, will use the "train" split when do_train is enabled'
        },
    )
    validation_split_name_custom: Optional[str] = field(
        default=None,
        metadata={
            "help": 'The name of the validation split in the input dataset. If not specified, will use the "validation" split when do_eval is enabled'
        },
    )
    test_split_name_custom: Optional[str] = field(
        default=None,
        metadata={
            "help": 'The name of the test split in the input dataset. If not specified, will use the "test" split when do_predict is enabled'
        },
...
def main():
    ...
    # I mainly want to load train file and split, validation file and split, test file and split
    
    # this is another modification
    # if I want to use multiple dataset and split for training,
    # I should customize the train_dataset_name_custom as A-with-B-with-C
    # and train_split_name_custom as A-with-B-with-C
    
    if "-with-" not in custom_args.train_dataset_name_custom and \
       "-with-" not in custom_args.train_split_name_custom:
        raw_datasets_train = load_dataset(
            custom_args.train_dataset_name_custom,
            split=custom_args.train_split_name_custom
        )
    else:
        from datasets import concatenate_datasets
        train_dataset_names = custom_args.train_dataset_name_custom.split("-with-")
        train_split_names = custom_args.train_split_name_custom.split("-with-")
        raw_datasets_train = concatenate_datasets(
            [load_dataset(
                dataset_name,
                split=split_name
            ) for dataset_name, split_name in zip(train_dataset_names, train_split_names)]
        )
    
    raw_datasets_validation = load_dataset(
        custom_args.validation_dataset_name_custom,
        split=custom_args.validation_split_name_custom
    )
    raw_datasets_test = load_dataset(
        custom_args.test_dataset_name_custom,
        split=custom_args.test_split_name_custom
    )
    ...
    logger.info(raw_datasets)
    wandb.init(
        project="cwi-score-v2",
        name=training_args.run_name,
    )
    
    wandb.config.update({"custom_args": dataclasses.asdict(custom_args)})
    wandb.config.update({"data_args": dataclasses.asdict(data_args)})
    wandb.config.update({"model_args": dataclasses.asdict(model_args)})
    wandb.config.update({"training_args": dataclasses.asdict(training_args)})
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            result = metric.compute(
                predictions=pwreds, references=p.label_ids, average="micro")
        else:
        ...
        if 'pearsonr' in result:
            result['addition_pearsonr'] = pearsonr(p.label_ids, preds)[0]
            result['addition_pearsonr_pvalue'] = pearsonr(p.label_ids, preds)[
                1]
            result['addition_spearmanr'] = spearmanr(p.label_ids, preds)[0]
            result['addition_spearmanr_pvalue'] = spearmanr(p.label_ids, preds)[
                1]
    ...
    if training_args.do_eval:
        ...
        #     metrics = trainer.evaluate(eval_dataset=raw_datasets["test"])
    ...
    if training_args.do_predict:
        ...
        wandb.log({"predict": metrics})
        for k, v in metrics.items():
            wandb.run.summary["{}/{}".format('predict', k[len("predict_"): ])] = v
        ...
        if is_regression:
            ...
            predictions = np.array([np.where(p > 0, 1, 0)
        ...
        # I also want to predict on validation set
        logger.info("*** Predict on validation ***")
        # Removing the `label` columns if exists because it might contains -1 and Trainer won't like that.
        # if "label" in predict_dataset.features:
        #     predict_dataset = predict_dataset.remove_columns("label")
        predictions, _, metrics= trainer.predict(
            test_dataset = raw_datasets["validation"], metric_key_prefix="predict_on_validation")
        ...
        if is_regression:
            ...
            predictions = np.argmax(predictions, axis=1)
        output_predict_file = os.path.join(
            training_args.output_dir, "predict_results_validation.txt")
        if trainer.is_world_process_zero():
            with open(output_predict_file, "w") as writer:
                logger.info("***** Predict on validation results *****")
                writer.write("index\tprediction\n")
                for index, item in enumerate(predictions):
                    if is_regression:
                        writer.write(f"{index}\t{item:3.3f}\n")
